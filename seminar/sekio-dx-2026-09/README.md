...
> **本番の後半デッキは、このフォルダの `DXマーケティング後半編.pptx`（36枚）です。**（2026-09-09 決定）
> 別のチャンネルで作られた28枚版がありますが、そちらは使いません。
> 台本・配布物のページ番号も、すべてこの36枚に合わせてあります。
...
| `引き継ぎ.md` | **別のチャンネルで続きをやるとき、これを最初に貼る。**置き場所・決定事項・残作業・環境の落とし穴 |
...
1. **30枚目の事例3件**（「（事例1）」「（やったこと）」「（結果）」のチップ。1枠12文字以内）。数字は自分で確認できるものだけ。「保存2倍」より「月3人の予約が8人に」のようにお客さんの数で
2. 2枚目の部署名「可動産部／まちづくり部／AI実装部」は仮置き。実際の呼び方に差し替え
3. 8枚目の本文（AIに自分の分身）は、サイトの記述から起こした仮の文。実際にやっていることに直す
4. 24枚目の Instagram のボタン名（「詳細設定」「日時を指定」）は、当日朝に自分のスマホで確認
5. さとうみどり氏に確認：Wi-Fi／スマホ画面の投影可否／机の配置／参加者の年齢層・業種／配布資料の印刷分担と部数
