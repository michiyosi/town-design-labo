...
(function signs(){
 ...
 /* 板のサムネ（1枚目の写真）が他の板とかぶらないように、持っている写真の中で
    まだ使っていないものを先頭に回す。同じ写真の板が並ぶと、同じ板が繰り返して
    いるように見えるため。写真は減らさず、順番だけ変える */
 var usedLead = {};
 function leadFirst(im){
  if(!im || !im.length) return im || [];
  for(var k=0;k<im.length;k++) if(!usedLead[im[k]]){ usedLead[im[k]] = 1; return im.slice(k).concat(im.slice(0,k)); }
  return im;
 }
