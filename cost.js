...
(function () {
  ...
  var FIXED_ASSET_MIN  = 1500000; // 免税点（課税標準額）
  ...
  function calc() {
    ...
    if (route === 'vehicle') {
      ...
      lines.push({ n: carTaxLabel, v: carTax, s: (b.kind === 'truck' ? '軽貨物（黄色ナンバー・自家用）を前提とした標準税率です。普通貨物ベースの車両なら自動車税（種別割）が最大積載量に応じてかかります。' : '標準税率。都道府県が条例で定めるため地域差があります。牽引する側のトラクタの自動車税（小型10,200円・普通20,600円）は別途かかります。') });
      annual += carTax;
      ...
      if (b.kind === 'truck') {
        ...
        lines.push({ n: '自賠責保険（12か月）', v: CALI_TRAILER,
          s: '被けん引自動車の基準料率（2023年1月届出／離島・沖縄を除く）。2026年11月1日から改定予定。' });
        annual += CALI_TRAILER;
        lines.push({ n: '自動車重量税・検査手数料', v: null,
      ...
      var ftax = kazei >= FIXED_ASSET_MIN ? kazei * FIXED_ASSET_RATE : 0;
      lines.push({ n: '固定資産税（家屋）', v: ftax,
        s: '標準税率1.4％。ここでは本体価格の70％を仮の課税標準額として計算しています。実際の評価額は市町村が再建築価格方式で算定するため、この金額とは異なります。課税標準額150万円未満は免税点により課税されません。' });
      annual += ftax;
