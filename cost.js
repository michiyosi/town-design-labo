...
(function () {
  ...
  function calc() {
    ...
    if (biz) {
      ...
      if (monthly > 0) {
        ...
        h += '<div class="cs-card is-key"><span>初期投資の回収</span><strong>' + m + 'か月</strong><em>' +
             (y > 0 ? y + '年' + (mm ? mm + 'か月' : '') : mm + 'か月') + (etc === 0 ? '・初期費用は算出済み分のみ' : '') + '</em></div>';
      } else {
    ...
    h += row('陸送・登録・設置工事など', etc ? yen(etc) : '未入力', '搬入経路・距離・現地条件で変わります。ページ下部に公開料金表にもとづく目安を掲載しています。近い額を入力欄に入れると総額に反映されます。');
    h += '<div class="cs-row cs-total"><div>合計' + (etc === 0 ? '（陸送・登録・設置工事などは未入力）' : '') + '</div><div>' + yen(initial) + '</div></div>';
    h += '</div>';
    ...
    h += '<div class="cs-note"><strong>この試算は、意思決定の材料です。</strong>公表されている税率・保険料率と、ご入力いただいた条件をもとに計算しています。' +
         '試算に含むのは、本体参考価格・ご入力いただいた追加費用・金額を算出できる法定の税と保険料です。車両重量や経過年数、規模・地域で決まる費用（自動車重量税・検査手数料、キッチンカーの自賠責保険料、建築確認申請・構造設計・基礎工事など）は金額を置けないため、内訳では「要確認」と表示し、年間合計には含めていません。陸送費・登録費・設置工事費は条件によって大きく変わるため既定値は空にしてあり、ページ下部の公開料金表にもとづく目安レンジを参考に入力してお使いください。' +
         '建築物として計画する場合の固定資産税は、本体価格の70％を仮の課税標準額とした参考値です（実際の評価額は市町村が算定します）。また、車両として扱われるか建築物として扱われるかで、かかる税がまるごと入れ替わります。' +
         'まずは<a href="/hantei.html">設置前チェック</a>で確認事項を整理してください。</div>';
